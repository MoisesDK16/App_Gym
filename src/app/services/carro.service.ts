...
export class CarroService {
  ...
  addToCarro(detalle: Detalle, producto: Productos) {
    ...
    if (itemIndex > -1) {
      ...
      this.myList.push({
        detalle: [detalle.cantidad > 0 ? detalle : { ...detalle, cantidad: 1, total: producto.precioVenta}],
        producto: [producto],
      });
  ...
  eliminarItem(detalle: Detalle) {
    ...
    if (itemIndex > -1) {
      this.subtotal.next(this.subtotal.value - this.myList[itemIndex].detalle[0].cantidad * this.myList[itemIndex].producto[0].precioVenta);
      this.myList.splice(itemIndex, 1);
    }
  ...
  asignarCarritoALista(): any{
    ...
    this.carrito.next(this.myList);
    return preFactura ? new BehaviorSubject<{ detalle: Detalle[], producto: Productos[] }[]>(this.myList) : this.carrito;
  }
